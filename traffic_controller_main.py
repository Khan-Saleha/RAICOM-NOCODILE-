...
class TrafficController:
    def __init__(self):
        ...
        self.model_path = os.path.join(os.path.dirname(__file__), 'yolo11n.pt')
        ...
        # Robot Control
        self.robot_speed = 0.02  # Normal speed
            ...
            if result.boxes is not None:
                for i, box in enumerate(result.boxes):
                    ...
                    # Zebra Crossing Detection
                        ...
                        self.draw_zebra_crossing_detection(cv_image, int(x1), int(y1), int(x2), int(y2), class_name, confidence)
    ...
    def draw_zebra_crossing_detection(self, image, x1, y1, x2, y2, class_name, confidence):
        ...
        label = f"ZEBRA CROSSING: {confidence:.2f}"
        cv2.putText(image, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    ...
    def publish_visualization_image(self, image):
        ...
        if self.zebra_crossing_detected:
            cv2.putText(image, "ZEBRA CROSSING - SLOWING", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
